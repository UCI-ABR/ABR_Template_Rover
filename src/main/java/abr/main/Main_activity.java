		...
		CvCameraViewListener2 // implements IOIOLooperProvider: from IOIOActivity
{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		dest_loc.setLongitude(-117.842748);
		dest_loc.setLatitude(33.645938);
	    ...
	    if (isClient) {
	    	try {
	    		...
	    		Log.e("abr template", e.getMessage());
	    	}  	
	    }
	    else {
	    	try {
	    ...
	    */
		// phone must be Android 4.1 or higher and have Google Play store
    ...
    public void onConnected(Bundle connectionHint) {
		...
		try {
			...
			Log.e("abr template","Problem with location permissions");
		}
    }
	//Method necessary for google play location services
	...
	public String scan(Mat frame) {
	    ...
	    try {
			...
			try {
			    File newFolder = new File(Environment.getExternalStorageDirectory(), "ABR_Template_Rover");
	...
	public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
		//Log.i("abr template","area:"+(mDetector.getMaxArea()/(mDetector.getCenterX()*mDetector.getCenterY()*4)) );
		...
		if (autoMode) { // only move if autoMode is on and time under time limit
			 // add code here
			if(m_ioio_thread.get_ir2_reading() > 15){
				m_ioio_thread.set_speed(1500+50);
	...
	//send an integer to another robot
	public void sendInt(int int_to_send){
		if(dataOutputStream != null)
			try {
				dataOutputStream.writeInt(int_to_send);
				Log.i("abr template", "grid sent");
			} catch (IOException e) {
				Log.e("abr template", e.getMessage());
			}
	}

	//receive an integer from another robot
	public int getInt(){
		try {
